...
/**
 * Centros con catálogo de reservas habilitado (para UI compacta en portada).
 * @param {Array<Record<string, unknown>>} centers
 * @returns {{ id: string | number, name: string }[]}
 */
export function listCatalogEnabledCenters(centers) {
  return (Array.isArray(centers) ? centers : [])
    ...
    .map((c) => {
      ...
      const name =
        nameRaw ||
        (c.code != null && String(c.code).trim() !== "" ? String(c.code).trim() : "");
      const id = c.id != null ? c.id : name;
      return { id, name };
    })
    .filter((x) => x.name);
}
...
/**
 * Nombres de centros con catálogo (texto plano; evitar listas largas en párrafos).
 * @param {Array<Record<string, unknown>>} centers
 */
export function formatCatalogCenterList(centers) {
  const items = listCatalogEnabledCenters(centers);
