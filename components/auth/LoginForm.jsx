...
const fieldBase = `sambil-login-field min-h-11 w-full ${ROUNDED_CONTROL} border border-zinc-200 bg-zinc-50/80 px-3.5 py-2.5 text-base text-zinc-900 shadow-inner shadow-zinc-100/50 transition-[border-color,box-shadow,background-color] duration-200 ease-out placeholder:text-zinc-400 focus:border-[#0c9dcf]/45 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#0c9dcf]/18 sm:min-h-10 sm:text-sm`;
    ...
    <div className="relative flex min-h-[calc(100dvh-10rem)] flex-col lg:grid lg:min-h-[calc(100dvh-8.5rem)] lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
      <aside className="relative order-1 flex min-h-[11rem] flex-col justify-end overflow-hidden bg-zinc-950 px-6 py-8 text-white sm:min-h-[13rem] lg:order-none lg:min-h-0 lg:justify-between lg:px-12 lg:py-14 xl:px-16">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -left-[20%] -top-[30%] h-[22rem] w-[22rem] rounded-full bg-[#0c9dcf]/30 blur-[80px]" />
          ...
          <div className="relative">
            <p className="font-semibold tracking-tight text-white/95">
              <span className="text-2xl sm:text-3xl">Sambil</span>
          ...
          <p className="relative mt-6 hidden text-xs text-zinc-500 lg:mt-0 lg:block">
            Marketplace B2B · Cuentas habilitadas por el equipo Sambil
          </p>
      </aside>
  ...
  if (!authReady) {
    return (
      <LoginShell>
        ...
        >
          <span className="inline-block h-5 w-5 animate-pulse rounded-full bg-[#0c9dcf]/40" aria-hidden />
    ...
    <LoginShell>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Link
            href="/"
            className="shrink-0 text-xs font-medium text-zinc-500 underline-offset-4 transition-colors hover:text-[#0c9dcf] hover:underline"
          >
            ← Catálogo
          </Link>
        </div>
        ...
        <p className="mt-5 text-pretty text-sm leading-relaxed text-zinc-600">
          Necesario para completar el checkout y gestionar tu empresa. Las cuentas las habilita el equipo de
          Sambil junto con el alta de tu empresa.
        </p>
        ...
        <form onSubmit={onSubmit} className="mt-8 space-y-5">
          ...
          <div>
            ...
            <div className="relative mt-2">
              ...
              <button
                type="button"
                className="absolute inset-y-0 right-0 flex w-11 items-center justify-center rounded-r-[15px] text-zinc-500 transition-colors hover:bg-zinc-100/50 hover:text-zinc-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0c9dcf]/25"
